...
TextIndicator.prototype = {
    shoot: function(sprite, duration, distance, key){
        switch(key){
            case("KeyW"):
                this.x = this.stage.p4[0];
                this.y = this.stage.p4[1];
                break;
            case("ArrowUp"):
                this.x = this.stage.p5[0];
                this.y = this.stage.p5[1];
                break;
            default:
                log("You got to the default. How?")
        }
        this.indicators.create(this.xy.x, this.xy.y, sprite);
        ...
        this.m1.to({x:this.xy.x+distance,y:this.xy.y-distance, alpha:0.5}, duration,Phaser.Easing.Exponential.In);
